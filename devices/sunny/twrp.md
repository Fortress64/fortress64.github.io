---
subtitle: Xiaomi Redmi Note 10 (sunny/mojito)
title: TWRP — Download
---

![TWRP Logo](/img/logos/TWRP.webp)

> **Important:** Please read the installation guide first — it contains crucial information.
> [Xiaomi Firmware, ROM and Recovery Installation Guide](/manuals/xiaomi-installation/)

---

## Release Information

| | |
|---|---|
| **Device** | Xiaomi Redmi Note 10 (sunny / mojito) |
| **Recovery** | TWRP (Team Win Recovery Project) |
| **Release date** | July 10, 2025 |

---

## Changelog

*Changelog will be added with the next release.*

---

## Downloads

Each recovery is available as a bootable image or an installable ZIP. The recovery can be booted directly via Fastboot without being permanently flashed to the device.

| File | Description | Download |
|---|---|---|
| `twrp-sunny.img` | Bootable image | [Download](https://github.com/PTX64/releases_sunny_recoveries/releases/download/latest/twrp-sunny.img) |
| `twrp-sunny.zip` | Installable ZIP | [Download](https://github.com/PTX64/releases_sunny_recoveries/releases/download/latest/twrp-sunny.zip) |

---

## SHA256 Checksums

```
# Verify with: sha256sum -c checksums.sha256
# Checksums available on the GitHub release page
```

[View checksums on GitHub](https://github.com/PTX64/releases_sunny_recoveries/releases/tag/latest)

---

## Other Recovery Options

- [OrangeFox](/devices/sunny/orangefox/) — TWRP-based recovery with built-in Magisk manager support
- [Pitch Black Recovery Project (PBRP)](/devices/sunny/pbrp/) — Dark-themed recovery with clean UI

---

## Troubleshooting

Having boot issues after flashing? See the [Troubleshooting guide](/manuals/troubleshoot/#bootissues).

---

[← Back to device page](/devices/sunny/)---
title: TWRP Downloads for Sunny
---
